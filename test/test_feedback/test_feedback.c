...
DEFINE_FFF_GLOBALS;
FAKE_VALUE_FUNC(enum FeedbackState, feedback_get_state_callback, enum FeedbackName);

void setUp() {
    RESET_FAKE(feedback_get_state_callback);
    FFF_RESET_HISTORY();
    ...
    SET_RETURN_SEQ(feedback_get_state_callback, feedback_states, FEEDBACK_NAME_COUNT)

    feedback_init(feedback_get_state_callback);
}
...
void test_feedback_get_state_call_count(void) {
    ...
    TEST_ASSERT_EQUAL_INT_MESSAGE(
        FEEDBACK_NAME_COUNT,
        feedback_get_state_callback_fake.call_count,
